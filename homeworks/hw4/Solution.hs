...
local tmp (Reader ra) = Reader $ \r -> do
  let new_r = tmp r
  ra new_r
...
exampleCfg = BankConfig { interestRate = 0.05, transactionFee = 2, minimumBalance = 100 }
exampleAcc = Account { accountId = "A-001", balance = 1000 }
...
calculateInterest account = do
  ...
  pure $ truncate (rate * fromIntegral (balance account))
...
checkMinimumBalance account = do
  bal <- asks minimumBalance
  pure $ balance account >= bal
...
processAccount account = do
  ...
  flag <- checkMinimumBalance account
  pure (updated, interest, flag)
